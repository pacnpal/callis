...
- `SECRET_KEY` is auto-generated if not provided in `.env` and persisted to `/data/.secret_key` (`entrypoint.sh` uses `openssl rand -hex 32`; the API fallback uses `secrets.token_hex(32)`).
