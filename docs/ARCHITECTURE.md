...
- `AuthorizedKeysCommand` is set to `/etc/ssh/auth-keys.sh %u %f`, which makes an HTTP request to `http://localhost:8081/internal/keys/{username}` (with `X-Internal-Secret` header) and returns the active public keys for that user. The `%f` token carries the offered key's SHA-256 fingerprint, which the API uses to stamp the key's last-used time and write a `key_used` audit entry. OpenSSH runs `AuthorizedKeysCommand` with a sanitized environment, so the script resolves the internal secret by deriving it from the persisted `SECRET_KEY` (`/data/.secret_key`) rather than relying on an inherited env var.
- OS user accounts are pre-created by the reconciler `user-sync.sh`, which runs alongside `sshd`: at startup and then on a fixed interval it fetches the active users with keys from `http://localhost:8081/internal/users` and ensures an OS account exists for each. Pre-creation is required because OpenSSH will not invoke `AuthorizedKeysCommand` for a username that is missing from the OS user database — it rejects it as an invalid user first. `auth-keys.sh` also creates the account on demand as a fallback for the startup race.
- Accounts are created **unlocked** (shadow password `*`, not the locked `!` that `useradd`/`adduser` set by default) via `ensure-user.sh`; otherwise `sshd` refuses them with "account is locked" even for public-key auth.
